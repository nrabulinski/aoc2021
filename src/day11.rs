	...
	for _ in 0..100 {
        ...
        loop {
            ...
            if board[idx] == 10 {
                ...
                if x > 0 { idxs.push(idx - 1); }
                if x < SIZE - 1 { idxs.push(idx + 1); }
                if y > 0 { idxs.push(idx - SIZE); }
                if y < SIZE - 1 { idxs.push(idx + SIZE); }
                if x > 0 && y > 0 { idxs.push(idx - SIZE - 1); }
                if x < SIZE - 1 && y > 0 { idxs.push(idx - SIZE + 1); }
                if x > 0 && y < SIZE - 1 { idxs.push(idx + SIZE - 1); }
                if x < SIZE - 1 && y < SIZE - 1 { idxs.push(idx + SIZE + 1); }
            }
        }
